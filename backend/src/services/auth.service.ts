...
export class AuthService {
  ...
  async register(userData: RegisterRequest): Promise<{ user: IUser }> {
    ...
    if (existingUser) {
      throw new ApiError((400 as any).toString(), 'User with this email or username already exists');
    }
    ...
    if (userData.role === (ROLES as any).RECEPTIONIST || !userData.role) {
  ...
  async login(email: string, password: string): Promise<AuthResult> {
    ...
    if ((user as any).refreshTokens.length > 5) {
      (user as any).refreshTokens = (user as any).refreshTokens.slice(-5);
    }
  ...
  async refreshAccessToken(refreshToken: string): Promise<{ accessToken: string }> {
    try {
      ...
      if (!user) {
        throw new ApiError((401 as any).toString(), 'Invalid refresh token');
      }
  ...
  async getUserById(userId: string): Promise<IUser> {
    ...
    if (!user) {
      throw new ApiError((404 as any).toString(), 'User not found');
    }
  ...
  async changePassword(userId: string, currentPassword: string, newPassword: string): Promise<void> {
    ...
    if (!isCurrentPasswordValid) {
      throw new ApiError((400 as any).toString(), 'Current password is incorrect');
    }
  ...
  async resetPassword(token: string, newPassword: string): Promise<void> {
    try {
      ...
      if (!tokenValidation.valid) {
        throw new ApiError((400 as any).toString(), 'Invalid or expired reset token');
      }
  ...
  async verifyEmail(token: string): Promise<void> {
    try {
      ...
      if (!tokenValidation.valid) {
        throw new ApiError((400 as any).toString(), 'Invalid verification token');
      }
      ...
      if ((user as any).isVerified) {
        throw new ApiError((400 as any).toString(), 'Email already verified');
      }
      ...
      throw new ApiError((400 as any).toString(), 'Invalid or expired verification token');
    }
  }
  ...
  private async generateMRN(): Promise<string> {
    ...
    return `PT${year}${month}${String((counter as any).sequence).padStart(6, '0')}`;
  }
}